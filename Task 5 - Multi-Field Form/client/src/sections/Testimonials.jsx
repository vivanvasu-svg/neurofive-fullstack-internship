  ...
  const fetchReviews = async () => {
    try {
      ...
      if (data && data.length > 0) {
        // Map database review fields to match TestimonialCard props
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      await api.createReview({
        quote: newReview.trim(),
        rating: Number(newRating),
      })
      setNewReview('')
      setNewRating(5)
      fetchReviews() // Refresh testimonials
    } catch (err) {
      console.error(err)
      ...
      <div className="container">
        ...
        {isAuth ? (
          ...
          <div className={styles.loginMsg}>
            <p>Loved your coffee? <a href="/login" className={styles.loginLink}>Login</a> to share your review with the community!</p>
          </div>
        )}
      </div>
    </section>
